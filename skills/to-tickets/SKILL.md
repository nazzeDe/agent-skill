---
name: to-tickets
description: Split approved production-code work into small, independent, vertical, verifiable local agent tickets; docs/chores stay with parent; user approves before persist via the selected local artifact backend.
disable-model-invocation: true
---

# To Tickets

Short-lived agent execution contracts. Persist only via the selected artifact backend (`../../ARTIFACT-BACKENDS.md` if unresolved). Not remote trackers / public docs. The ticket is a document the worker will load; write it with `writing-for-agents`.
