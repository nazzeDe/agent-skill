...
Tailwind 和 Mermaid 均通过 CDN 引入。Mermaid 处理图拓扑结构的图表；手写 div 和内联 SVG 处理有社论/排版风格的视觉图表（体量图、横截面图）
...
包含仓库名称、日期以及一个紧凑的图例：实线框 = 模块（module），虚线 = 接缝（seam），红色箭头 = 泄漏（leakage），深色粗框 = 深层模块（deep module）。直接展示候选方案

## Candidate card

图表承担主要核心内容。正文精简、直白，直接使用词汇表中的术语
每个 candidate 对应一个 `<article>`:
...
- Use `text-xs uppercase tracking-wider` for module labels inside diagrams — they should read as schematic
